...
class StatusIndicator extends ConsumerWidget {
      ...
      child: Row(
        ...
        children: [
          ...
          _buildMiniBar(context,
              label: 'S',
              used: state.totalSessionUsed,
              limit: state.totalSessionLimit),
          const SizedBox(width: 6),
          _buildMiniBar(context,
              label: 'W',
              used: state.totalWeeklyUsed,
              limit: state.totalWeeklyLimit),
  ...
  Widget _buildMiniBar(BuildContext context,
      {required String label, required int used, required int limit}) {
      ...
      children: [
        Container(
          ...
          child: Align(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  color: level.color, borderRadius: BorderRadius.circular(2)),
            ),
          ),
        ),
        ...
        Text('$label ${(pct * 100).toInt()}%',
            style: Theme.of(context).textTheme.labelSmall?.copyWith(
                fontSize: 9, color: level.color, fontWeight: FontWeight.w600)),
      ],
    );
  }
}
